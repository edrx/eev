...
2.1. `M-x find-dot-emacs-links'
-------------------------------
The most obvious way to make your init file - a.k.a. your ~/.emacs -
load eev is by adding some lines to it by hand, as explained in the
section above; but the _best_ way is by using `find-dot-emacs-links'.
Try to execute each one of the sexps below with `M-2 M-e':

  (find-dot-emacs-links \"eev\")
  (find-dot-emacs-links \"eev angges\")
  (find-dot-emacs-links \"eev angges melpa\")
  (find-dot-emacs-links \"eev angges melpa mfms\")

You will see that the words in the argument, like `eev', `angges', etc,
determine what appears between the `ee-copy-rest-3' and the `;;--end'.
For example, in the argument is just \"eev\", you get this:
...
The details are explained here, with screenshots:

  https://anggtwu.net/2024-find-dot-emacs-links.html
  https://anggtwu.net/2024-find-dot-emacs-links.html#find-dot-emacs-links

In short: running the `ee-copy-rest-3' does not change your ~/.emacs,
but it copies some lines to the kill ring and shows your ~/.emacs at the
window at the right... then you can choose where to paste those lines,
paste them there with `C-y', and make any necessary adjustments - like
deleting a previous version of the initialization code for eev.
...
I believe that the best way to handle this would be to:
  ...
  1. prepare the list of the eev functions that the completion engine
     should hide by default, convert that list to a hash table, and:

  2. tell the completion package to not list the functions in that hash
     table.

At this moment I only have a part of (1) - it is in the uncommented part
of the temporary buffer generated by:

  (find-eloadhistory-links)

If you have a good notion of which functions you would like to hide and
you know at least on completion engine well, please get in touch - I
need your help!!! =/
...
THIS INTRO IS OBSOLETE!

See: (find-eev-levels-intro \"2.1. `M-x find-dot-emacs-links'\")
     http://anggtwu.net/2024-find-dot-emacs-links.html
